...
## creates three arrays of length n
## One to represent a particle at each site
## One to represent which sites are awake (set to True) currently
## One to represent whether the site will be awake in the future (all set to False initially)
@njit
def setUp(n: int):
    part_arr = np.ones(n, dtype=np.int16)
    state_arr = np.full(n, True, dtype=np.bool_)
    fut_state_arr = np.full(n, False, dtype=np.bool_)
    return part_arr, state_arr, fut_state_arr

## Goes through array and moves particles until stabilization (one asleep particle or none per site)
@njit
def runRound(part_arr, state_arr, fut_state_arr, addIndex, lenArr, directions):
    topple = True
    numTopples = 0
    time = 0
        ...
        for i in range(lenArr):
            while state_arr[i] == True and part_arr[i] > 0:
               ## if instruction list was empty, replenish, then grab instruction for index
                ...
                currInstr = directions[int(instr[instr_in]*len(directions))] ## generate instruction to move particle
                instr_in += 1
                ...
                if currInstr == 0 and part_arr[i] == 1: ## Site falls asleep
                    ...
                    if (i + currInstr) < (lenArr) and (i + currInstr) >= 0:
                        ...
                        fut_state_arr[i + currInstr] = True

            topple = True if roundTopples > 0 else False ## topple is false if no indices needed to be stabilized
        ## switch the future and current state arrays so we loop through the future one on the next run   
        foo = state_arr
        state_arr = fut_state_arr
        fut_state_arr = foo
...
def runNTrials(nParticles = 128, nTrials = 1000, lambda_rate = 0.5):
    part_arr, state_arr, fut_state_arr = setUp(nParticles)
    ...
    for i in range(nTrials):
        #density,
        topples, size, time = runRound(part_arr, state_arr, fut_state_arr, int(len(part_arr) / 2), len(part_arr), directions)
        #avgDens[i] = density
...
def data_collapse_plot(data_dict, obs, beta, tau, label, num_avalanches_total, rate):
    """
    Data collapse plot:
      x_scaled = x / L^beta
